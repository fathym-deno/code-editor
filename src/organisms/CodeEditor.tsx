...
export type CodeEditorHandle = {
  OpenFile: (fileKey: string, whenEmpty?: boolean) => void;
...
const CodeEditor = forwardRef<CodeEditorHandle, CodeEditorProps>(
  function CodeEditor(
    ...
    const handleFileRemove = (fileLookup: string): void => {
    ...
    useImperativeHandle(ref, () => ({
      OpenFile: (filePath, whenEmpty) => {
        if (!whenEmpty || !openFileLookups?.length) {
          const [codeSourceLookup, actionPath] = filePath.split('|');

          handleFileClick(codeSourceLookup, actionPath);
        }
      },
